...
const ContextWrapper = ({ children }: { children: ReactNode }) => {
    ...
    async function fetchApiForAllCartItems() {
        if (userData) {
            let res = await fetch(`/pages/api/cartfunc?user_id=${userData.uunid}`);
            if (!res.ok) {
    ...
    async function dispatch(payload: string, data: any) {
        if (payload === "addToCart") {
            ...
            await fetch(`/pages/api/cartfunc`, {
                method: "POST",
                body: JSON.stringify(data)
            });
        } else if (payload === "removeFromCart") {
            let dataa = await fetch(`/pages/api/cartfunc?product_id=${data.product_id}&user_id=${data.user_id}`, {
                method: "DELETE",
            });
            let NotData = await dataa.json();
        } else if (payload === "updateCart") {
            setLoading(true);
            let dataa = await fetch(`/pages/api/cartfunc`, {
                method: "PUT",
                body: JSON.stringify(data)
            });
            let NotData = await dataa.json();
